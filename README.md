...
Un motor de joc de tip Text-Based RPG implementat în C++, inspirat din universul Dark Souls. Proiectul simulează mecanici de explorare, luptă pe ture, gestionare a inventarului și interacțiune cu NPC-uri, demonstrând utilizarea conceptelor fundamentale și avansate ale Programării Orientate pe Obiecte.

Descriere Joc

Jucătorul preia rolul unui personaj care trebuie să exploreze o hartă interconectată , să învingă inamici și boși și să colecteze resurse.
...
1. Ierarhii de Clase
Au fost definite 3 ierarhii principale de clase:
...
Inventory<T>: Clasa template pentru gestionarea colecției de obiecte a jucătorului.
3. Polimorfism si RTTI
Utilizarea funcțiilor virtuale (attack, use, visit) pentru comportament polimorfic la rulare.
Utilizarea dynamic_cast (RTTI) pentru a determina tipul exact al obiectelor la runtime (de exemplu, verificarea dacă un Item este Weapon sau Consumable înainte de utilizare, sau tipul entității dintr-o cameră).
4. Gestiunea Resurselor si Memoriei
...
Utilizarea blocurilor try-catch pentru validarea input-ului utilizatorului și a acțiunilor critice (gestionarea inventarului).
6. STL (Standard Template Library)
std::vector: Utilizat extensiv pentru stocarea inventarului, a listei de vecini pentru fiecare cameră și pentru structura hărții (vector de vectori).
7. Membri Statici
Utilizarea membrilor statici pentru contorizarea instanțelor de entități sau boși activi în joc.
